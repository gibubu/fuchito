...
log4j = {
    ...
    //}
    appenders {
        console name:'stdout', layout:pattern(conversionPattern: '%d [FUCHITO] %5p [%c{2}] | %m%n')
    }
...
grails.plugins.springsecurity.onInteractiveAuthenticationSuccessEvent = { e, appCtx ->
    def domain = general.Usuario.executeQuery("select new map(usuario.equipo.torneo.nombre as torneo, usuario.equipo.nombre as equipo) from Usuario usuario where usuario.username = ?", [e.source.principal.username])
    def request = org.codehaus.groovy.grails.plugins.springsecurity.SecurityRequestHolder.getRequest()
    def session = request.getSession(false)
    session.torneo = domain[0].torneo
    session.equipo = domain[0].equipo
}
