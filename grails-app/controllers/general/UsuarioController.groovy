...
//		[usuarios: Usuario.findAllByEquipo(currentUser.equipo, params), totalDeUsuarios: Usuario.countByEquipo(currentUser.equipo)]
...
//            def currentUser = springSecurityService.currentUser
//            usuario.equipo = currentUser.equipo
...
//                usuario.equipo = currentUser.equipo
    ...
    def perfil = {
        ...
        if (SpringSecurityUtils.ifAnyGranted('ROLE_ADMIN')) {
            log.debug("Buscando equipos a nivel administrador")
            equipos = Equipo.findAll("from Equipo e order by e.torneo.nombre, e.nombre")
        } else if(SpringSecurityUtils.ifAnyGranted('ROLE_ORG')) {
            log.debug("Buscando equipos a nivel torneo")
            equipos = Equipo.findAll("from Equipo e where e.torneo = :torneo order by e.torneo.nombre, e.nombre", [torneo:usuario.equipo.torneo])
        } else {
            log.debug("Asignando equipo")
            equipos = [usuario.equipo]
        }
        
        return [usuario:usuario, equipos:equipos]
    }
    ...
    def actualizaPerfil = {
        Usuario.withTransaction {
            ...
            if (usuario) {
                ...
                // a un equipo / torneo no permitida
                if (!usuario.hasErrors() && usuario.save(flush: true)) {
                    session.torneo = usuario.equipo.torneo
                    session.equipo = usuario.equipo
